...
if __name__ == "__main__":
    ...
    # 保存
    df.to_csv(f"{args.output}-{args.task}.csv", index=False, float_format="%.4f")
    print(f"\n保存: {args.output} ({len(df)} 条记录)")
    
    if args.collect_type == CollectTy.MULTI_SEED:
        group_keys = ["Dataset"] 

        agg = aggregate_multiseed(records, group_keys)

        agg.to_csv(f"{args.output}-{args.task}-agg.csv", index=False)
        print(f"\n保存聚合结果: {args.output}-{args.task}-agg ({len(agg)} 条记录)")
