# E-2 消融（⚠️ 已作废：CN 伪交集旧语义 · 全量 20/20）

> ⚠️ **本表数字已作废，请勿引用（含 C5 素材）**：本批完成于 **CN 伪交集修复之前**（该修复 2026-09-11 晚部署）。**CN 真交集修复版重跑已排入队列（任务 #14/#15，20 runs：5 数据集 × 4 配置）**，完成后将整体替换本文件。历史代际：v1 含 pos0 泄漏 → 作废；v2 = 本表（泄漏/RAE 已修、CN 未修）→ 作废；**v3 = CN 修复版（待跑）→ 论文口径**。
> 协议：**linksign**（link&sign 3 类）；5 数据集；seed 42；消融定稿超参 NN-Best·LF-Best；**P1**、**TE**；**GPU（cuda:0/1）**；代码 = 泄漏/RAS/RAE 修复后（2026-09-09 fix，部署校验通过）+ **CN 伪交集旧语义**。
