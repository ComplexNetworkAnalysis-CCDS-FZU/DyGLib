...
| 日期 | 发出方 | 状态 | 内容 |
|---|---|---|---|
| 2026-09-09 | B | ✅ | **修复已验证（GPU）**：RAS/RAE/泄漏修复已实现并部署；BitcoinAlpha linksign full **修复后 = 修复前**（AUC 0.9649 等 6 项 4 位小数全同）→ 该数据集上泄漏移除无可测影响（泄漏信息冗余）。RAS/RAE 激活后边际贡献待修复后 E-2 消融确认。→ 若全数据集验证均无塌陷，主结论稳健；全量重跑为最终口径 |
