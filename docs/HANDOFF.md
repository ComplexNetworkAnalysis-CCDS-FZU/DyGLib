...
- **→ Perf 🔄**：①真实数据复测 ✅；②M4 方案审批①–④落定；③**服务器部署 + 构建校验就绪**：通道 ✅ / 环境 ✅（rustup 1.98.1、maturin 1.15.0）/ 首构建 ✅（sha `86c2e85f…`）/ **第 1 批校验全绿**（cargo 21/21、check_accel 12/12、上游 1800+908 全 PASS、bench 14/14；服务器口径 K1 7–11×、K2 50–285×）；**剩余**：crate `__abi__`（Perf 补）+ DyGLib 适配器（待批准）→ 门 2/3
