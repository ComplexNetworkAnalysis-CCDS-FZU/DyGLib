...
- **→ Perf 🔄**：①真实数据复测 ✅；②M4 方案审批①–④落定；③**服务器部署就绪**：通道 ✅（`~/git/SignDyG-Perf.git` + 工作拷贝 @ `9b4913b`）+ 环境 ✅（rustup/cargo 1.98.1、maturin 1.15.0，与本地同版）+ **首构建 ✅**（wheel sha256 `86c2e85f…`；gc 安装 + import ✓）；**剩余**：crate `__abi__`（Perf）+ DyGLib 适配器（M4 落地，待批准）→ 门 2/3
