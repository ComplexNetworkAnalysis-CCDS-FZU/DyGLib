...
| 日期 | 发出方 | 状态 | 内容 |
|---|---|---|---|
| 2026-09-09 | B | ⬜ | 【请 A 提供 · C5】RAS/RAE **权威语义请求**：请 A 摘录论文中 **RAS（repeat-aware sampler）与 RAE（repeat-aware sign encoder）的准确定义**（章节/公式/算法流程/输入输出），作为代码对齐基准。背景：B 审计确认当前代码 RAS 仅"自环"触发、RAE no-op，与论文意图很可能有偏差（用户 09-09 定：以论文定义为准，git 溯源跳过；B 不再自行解读论文 tex，以 A 摘录为准）。A 摘录后将据此 ①定修复方案或 ②改论文表述，并解除主表重跑阻塞 |
