...
| 日期 | 发出方 | 状态 | 内容 |
|---|---|---|---|| 2026-09-09 | B | ⬜ | 【请 A 提供 · C5】RAS/RAE **权威语义请求**：请 A 摘录论文中 **RAS（repeat-aware sampler）与 RAE（repeat-aware sign encoder）的准确定义**（章节/公式/算法流程/输入输出），作为代码对齐基准。背景：B 审计确认当前代码 RAS 仅"自环"触发、RAE no-op，与论文意图很可能有偏差（用户 09-09 定：以论文定义为准，git 溯源跳过；B 不再自行解读论文 tex，以 A 摘录为准）。A 摘录后将据此 ①定修复方案或 ②改论文表述，并解除主表重跑阻塞 || 2026-09-09 | B | ⬜ | 【请 A 注意 · C5 条件性】此前告知"E-3/E-4/E-5 不受影响"**仅在 C5=方案②（full:=BTE+CNAS，不修 RAS/RAE）下成立**。若 C5=方案①（按论文修好 RAS/RAE），full 模型改变 → **E-3（已入论文 §4.5）/E-4/E-5/主表全部需重跑**。B 已暂缓 E-4/E-5 正式汇总（原始数据已归档本地，C5 定后立即可汇或重跑）。请 A 知悉 E-3 入文的条件性
| 2026-09-09 | B | ⬜ | **C5 修复语义已定义并同步**：`docs/DESIGN_RAS_RAE_FIX.md` —— 确认**当前实现不能把 u–v 重复交互作为额外补充采样点**；RAS 正确触发条件应为 `v∈N(u)`（非自环）；RAE 应编码重复历史量化信号（次数/最近符号/一致性）；实现机制 A（采样器补锚点）/ B（独立特征流）与 4 个开放问题待导师/A 确认。**已按 A 09-09 意见暂缓主表重跑**；E-4/E-5 汇总将先行交付 |
