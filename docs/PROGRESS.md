...
- **2026-09-14 凌晨（续2·CN 重构评估，用户要求）**：新增 `tools/verify/cn_bincount_bench.py`——**bincount 直方图重构 vs 现行 numpy 版**：**不如现行**（BA 口径 1.29ms→3.24ms＝**0.40×**；全配置中位 0.42×，仅 B=18 尾批 1.73×；逐位一致全过；大 id 另有**域膨胀爆内存**缺陷：直方图域=B·n_nodes）。含 AVX 推论：CN 整 run 占比已 ~0.4–0.5%，真 SIMD 再快 3–5× 也仅再省 ~0.3% ⇒ **CN 加速以现行 numpy 版收官，不再投入 bincount/AVX 子路线**；K3-CN 的 ROI 相应降级（拟同步 `Perf`）。
