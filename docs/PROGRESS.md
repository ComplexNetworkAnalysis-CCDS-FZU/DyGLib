...
- **2026-09-13（傍晚）**：**补充实验进展**：ⓐ **P 补全 12/12 完成并同步**（BA/OTC/RT × P{1,3,5,7}）；全数据集 P 表引入 `results/E-3_patch/E3_patch_summary.md`（R2-11 数据齐）。ⓑ BTE-off 对照 2/10（BA 完成、OTC 在跑）；ⓓ 邻域网格已启动（#19 RT 在跑）。队列双卡连续，预计 ⓑ 深夜、ⓓ 明晨收齐。
