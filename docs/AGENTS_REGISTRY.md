...
| `Baseline` | DynamiSE/DySDGNN 基线复现 | `D:\codes\DynamiSE_DySDGNN_repro` | R2-5 方案① 复现；不改 DyGLib，数据只读 `processed_data/`；⛔ **禁服务器，只能本地**（服务器侧动作走 `Code`+用户许可） | 给 Baseline | C |
| `Perf` | Rust + PyO3 内核加速（用户称「路线 C」） | `D:\codes\SignDyG-Perf`（2026-09-11 创建） | ⛔ **不得接触服务器，只能本地运行**（未经用户逐次明确许可；含自动运行 agent；闸门达标≠许可）；DyGLib **只读** | 给 Perf | （2026-09-11 新加入） |
