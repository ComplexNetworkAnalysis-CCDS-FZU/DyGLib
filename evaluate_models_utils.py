...
def evaluate_edge_bank_link_prediction(
    ...
    for run in range(args.num_runs):
        ...
        for batch_idx, test_data_indices in enumerate(test_idx_data_loader_tqdm):
            ...
            batch_src_node_ids, batch_dst_node_ids, batch_node_interact_times = (
                test_data.src_node_ids[test_data_indices],
                test_data.dst_node_ids[test_data_indices],
                test_data.node_interact_times[test_data_indices],
            )

            if test_neg_edge_sampler.negative_sample_strategy != "random":
                batch_neg_src_node_ids, batch_neg_dst_node_ids = (
                    test_neg_edge_sampler.sample(
                        size=len(batch_src_node_ids),
                        batch_src_node_ids=batch_src_node_ids,
                        batch_dst_node_ids=batch_dst_node_ids,
                        current_batch_start_time=batch_node_interact_times[0],
            ...
            positive_probabilities, negative_probabilities = edge_bank_link_prediction(
            ...
            predicts = torch.from_numpy(
                np.concatenate([positive_probabilities, negative_probabilities])
            ).float()
            labels = torch.cat(
                [
                    torch.ones(len(positive_probabilities)),
                    torch.zeros(len(negative_probabilities)),
                ],
                dim=0,
            )
