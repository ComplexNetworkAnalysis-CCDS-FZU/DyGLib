...
class NeighborSampler:
    ...
    def __init__(
        ...
        for node_idx, per_node_neighbors in enumerate(adj_list):
            ...
            self.nodes_neighbor_signs.append(
    ...
    def find_neighbors_before(
        ...
        if return_sampled_probabilities:
            return (
                ...
                self.nodes_neighbor_signs[node_id][:i],
    ...
    def get_historical_neighbors(
        ...
        # 每个在位置（i,j）的内容表示交互符号在src节点node_ids[i] 和 目标节点 nodes_neighbor_ids[i][j] 的符号，在 node_interact_times[i]之前
        # ndarray, shape (batch_size, num_neighbors)
        nodes_neighbor_sign = np.zeros((len(node_ids), num_neighbors)).astype(np.int8)
        ...
        for idx, (node_id, node_interact_time) in enumerate(
            ...
            if len(node_neighbor_ids) > 0:
                if self.sample_neighbor_strategy in ["uniform", "time_interval_aware"]:
                    ...
                    nodes_neighbor_sign[idx, :] = node_neighbor_sign[sampled_indices]
                    ...
                    nodes_neighbor_sign[idx, :] = nodes_neighbor_sign[idx, :][
                        sorted_position
                    ]
                    ...
                    node_neighbor_sign = node_neighbor_sign[-num_neighbors:]
                    ...
                    nodes_neighbor_sign[
                        idx, num_neighbors - len(node_neighbor_sign) :
                    ] = node_neighbor_sign
                else:
                    raise ValueError(
...
def get_neighbor_sampler(
    ...
    for (
        ...
        adj_list[src_node_id].append(
            (dst_node_id, edge_id, node_interact_time, node_interact_sign)
        )
        adj_list[dst_node_id].append(
            (src_node_id, edge_id, node_interact_time, node_interact_sign)
        )
